...
var ZKFPLoader = class {
  ...
  resolveDllPaths(mainDllName) {
    ...
    if (mainDllName) {
      if (mainDllName === "libzkfp.dll") {
        const demoLibPath = (0, import_path.join)(process.cwd(), "demotest", "libzkfp.dll");
      ...
      const dllDirs2 = [
        (0, import_path.join)(process.cwd(), "src", "dll", "x64"),
        (0, import_path.join)(process.cwd(), "src", "dll", "x86")
      ];
      for (const dllDir of dllDirs2) {
        const dllPath = (0, import_path.join)(dllDir, mainDllName);
  ...
  fileExists(path) {
    try {
      (0, import_fs.accessSync)(path, import_fs.constants.R_OK);
      return true;
    } catch {
      return false;
    }
